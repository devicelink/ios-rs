...
use usbmux_sim::{SimDevice, UsbmuxSim};
...
fn list_devices_one_usb() {
    ...
    assert_eq!(devices[0].connection_type, usbmux::ConnectionType::Usb);
...
fn list_devices_mixed() {
    ...
    assert_eq!(usb.connection_type, usbmux::ConnectionType::Usb);
    assert_eq!(net.connection_type, usbmux::ConnectionType::Network);
}
...
fn listen_receives_initial_attach_events() {
    ...
    let mut serials: Vec<String> = vec![
        match ev1 { usbmux::Event::DeviceAttached(d) => d.serial, _ => panic!("not attached") },
        match ev2 { usbmux::Event::DeviceAttached(d) => d.serial, _ => panic!("not attached") },
    ];
    serials.sort();
