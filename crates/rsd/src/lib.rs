...
impl RsdClient {
    ...
    pub fn connect_stream(stream: std::os::unix::net::UnixStream) -> Result<Self, Error> {
        ...
        let listener   = TcpListener::bind("127.0.0.1:0").map_err(remotexpc::Error::Io)?;
        let local_addr = listener.local_addr().map_err(remotexpc::Error::Io)?;
