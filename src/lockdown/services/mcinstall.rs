...
pub const SKIP_SETUP_KEYS: &[&str] = &[
    "Accessibility", "AccessibilityAppearance", "ActionButton",
    "AgeAssurance", "AgeBasedSafetySettings", "Android",
    "Appearance", "AppleID", "AppStore", "Avatar",
    "Biometric", "CameraButton", "CloudStorage",
    "DeviceProtection", "DeviceToDeviceMigration", "Diagnostics",
    "Display", "EnableLockdownMode", "ExpressLanguage",
    "FileVault", "iCloudDiagnostics", "iCloudStorage",
    "iMessageAndFaceTime", "IntendedUser", "Intelligence",
    "Keyboard", "Language", "LanguageAndLocale", "Location",
    "LockdownMode", "MessagingActivationUsingPhoneNumber",
    "Multitasking", "OSShowCase", "Passcode", "Payment",
    "PreferredLanguage", "Privacy", "Region", "Registration",
    "Restore", "RestoreCompleted", "Safety", "SafetyAndHandling",
    "ScreenSaver", "ScreenTime", "SIMSetup", "Siri",
    "SoftwareUpdate", "SpokenLanguage", "TapToSetup",
    "TermsOfAddress", "Tips", "Tone", "TOS", "TouchID",
    "TrueToneDisplay", "TVHomeScreenSync", "TVProviderSignIn",
    "TVRoom", "UnlockWithWatch", "UpdateCompleted",
    "Wallpaper", "WatchMigration", "WebContentFiltering",
    "Welcome", "WiFi",
];
...
impl McInstallClient {
    ...
    pub fn get_cloud_config(&mut self) -> Result<plist::Dictionary, Error> {
        let mut req = plist::Dictionary::new();
        req.insert("RequestType".into(), Value::String("GetCloudConfiguration".into()));
        self.send(&Value::Dictionary(req))?;
        self.recv()?.as_dictionary().cloned()
            .ok_or_else(|| Error::Lockdown("MCInstall: GetCloudConfiguration response not a dict".into()))
    }

    /// Perform the host-identifier handshake.
    pub fn hello(&mut self) -> Result<(), Error> {
        let mut req = plist::Dictionary::new();
        req.insert("RequestType".into(), Value::String("HelloHostIdentifier".into()));
    ...
    pub fn set_cloud_config(&mut self, config: plist::Dictionary) -> Result<(), Error> {
        let mut req = plist::Dictionary::new();
        req.insert("RequestType".into(),      Value::String("SetCloudConfiguration".into()));
    ...
    pub fn escalate(&mut self, supervisor_cert_der: &[u8]) -> Result<Vec<u8>, Error> {
        ...
        req.insert("SupervisorCertificate".into(), Value::Data(supervisor_cert_der.to_vec()));
        self.send(&Value::Dictionary(req))?;
        let resp = self.recv()?;
        ...
        match dict.get("Challenge") {
            ...
            _ => Err(Error::Lockdown("MCInstall Escalate: no Challenge in response".into())),
        }
    }
    ...
    pub fn escalate_response(&mut self, signed: &[u8]) -> Result<(), Error> {
        let mut req = plist::Dictionary::new();
        req.insert("RequestType".into(),  Value::String("EscalateResponse".into()));
    ...
    pub fn proceed_keybag_migration(&mut self) -> Result<(), Error> {
        let mut req = plist::Dictionary::new();
        req.insert("RequestType".into(), Value::String("ProceedWithKeybagMigration".into()));
    ...
    fn recv(&mut self) -> Result<Value, Error> {
        ...
        if len == 0 || len > 8 * 1024 * 1024 {
            return Err(Error::Lockdown(format!("MCInstall: bad response length {len}")));
        }
...
fn check_status(dict: &plist::Dictionary, op: &str) -> Result<(), Error> {
    match dict.get("Status").and_then(|v| v.as_string()) {
        ...
        Some(s) => {
            let detail = dict.get("Error")
                .and_then(|v| v.as_string())
                .map(|e| format!(": {e}"))
                .unwrap_or_default();
            Err(Error::Lockdown(format!("MCInstall {op}: status {s:?}{detail}")))
        }
    }
}
