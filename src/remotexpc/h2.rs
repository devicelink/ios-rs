...
pub fn read_frame<R: Read>(r: &mut R) -> Result<Frame, Error> {
    ...
    let frame = match frame_type {
        ...
        TYPE_GOAWAY => {
            let last_stream =
                u32::from_be_bytes(payload.get(0..4).unwrap_or(&[0; 4]).try_into().unwrap())
                    & 0x7fff_ffff;
            let error =
                u32::from_be_bytes(payload.get(4..8).unwrap_or(&[0; 4]).try_into().unwrap());
            Frame::GoAway { last_stream, error }
        }
        TYPE_WINDOW_UPDATE => {
            let increment =
                u32::from_be_bytes(payload[..4].try_into().unwrap_or([0; 4])) & 0x7fff_ffff;
            Frame::WindowUpdate {
                stream_id,
                increment,
            }
