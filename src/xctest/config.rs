...
pub fn build_xctest_configuration_bytes(args: XCTestConfigArgs<'_>) -> Vec<u8> {
    ...
    let automation_path_uid = push_str(
        &mut objects,
    ...
    let ide_caps_uid = {
        ...
        for k in &cap_keys_1 {
            key_uids.push({
                let u = Uid::new(objects.len() as u64);
        ...
        key_uids.push({
            let u = Uid::new(objects.len() as u64);
            objects.push(Value::String("test iterations".into()));
            u
        });
    ...
    cfg.insert(
        "defaultTestExecutionTimeAllowance".into(),
        Value::Uid(Uid::new(0)),
    );
    ...
    cfg.insert(
        ...
        Value::Boolean(args.initialize_for_ui),
    );
    cfg.insert(
        "maximumTestExecutionTimeAllowance".into(),
        Value::Uid(Uid::new(0)),
    );
    cfg.insert(
        "randomExecutionOrderingSeed".into(),
        Value::Uid(Uid::new(0)),
    );
    ...
    cfg.insert(
        "testApplicationUserOverrides".into(),
        Value::Uid(Uid::new(0)),
    );
