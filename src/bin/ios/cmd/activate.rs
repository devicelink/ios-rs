...
pub fn activate(udid: Option<&str>, mode: ConnectionMode, output: OutputMode) -> Result<()> {
    ...
    plist::to_writer_xml(
        &mut session_info_xml,
        &plist::Value::Dictionary(session_info),
    )
    ...
    let activation_info = client
        .create_activation_info(&drm_bytes)
        .context("CreateActivationInfoRequest")?;

    let mut activation_info_xml = Vec::new();
    plist::to_writer_xml(
        &mut activation_info_xml,
        &plist::Value::Dictionary(activation_info),
    )
    ...
    client
        .handle_activation_with_session(&activation_response, headers)
