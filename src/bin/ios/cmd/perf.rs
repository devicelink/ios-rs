        ...
        match sample_once(&mut session, &config) {
            Ok(snap) => {
                ...
                if json_mode {
                    ...
                    print_htop(&snap, cols, rows);
                }
            }
            Err(e) => {
                consecutive_failures += 1;
                if !json_mode {
                    print_status(&format!("reconnecting… ({e})"), cols);
                }
                eprintln!("[perf] {e}");
            }
        }
...
fn sample_once(session: &mut ios_rs::tunnel::DeviceSession, config: &[u8]) -> Result<Snapshot> {
    let conn = connect_hub(session).map_err(|e| anyhow::anyhow!("connect: {e}"))?;
    conn.handshake()
    ...
    let tap = conn
        .request_channel("com.apple.instruments.server.services.sysmontap")
...
fn connect_hub(session: &mut ios_rs::tunnel::DeviceSession) -> Result<Arc<DtxConn>> {
    ...
    let stream = session
        ...
        .map_err(|e| anyhow::anyhow!("dtservicehub (is Developer Mode enabled?): {e}"))?;
    let stream_r = stream
        .try_clone()
...
fn build_config(interval_ms: u64, proc_attrs: &[&str], sys_attrs: &[&str]) -> Vec<u8> {
    ...
    let make_arr = |objects: &mut Vec<Value>, strs: &[&str]| -> plist::Uid {
        let uids: Vec<plist::Uid> = strs
            .iter()
            .map(|s| push_val(objects, Value::String((*s).into())))
            .collect();
        let idx = plist::Uid::new(objects.len() as u64);
        let mut d = Dictionary::new();
        d.insert(
            "NS.objects".into(),
            Value::Array(uids.iter().map(|u| Value::Uid(*u)).collect()),
        );
    ...
    let root = plist::Uid::new(objects.len() as u64);
    {
        ...
        d.insert(
            "NS.keys".into(),
            Value::Array(
                [k_ur, k_bm, k_si, k_cpu, k_phys, k_proc, k_sys]
                    .iter()
                    .map(|u| Value::Uid(*u))
                    .collect(),
            ),
        ...
        d.insert(
            "NS.objects".into(),
            Value::Array(
                [v_ur, v_bm, v_si, v_true, v_true, v_proc, v_sys]
                    .iter()
                    .map(|u| Value::Uid(*u))
                    .collect(),
            ),
...
fn extract_snapshot(msg: &dtx::DtxMessage) -> Option<Snapshot> {
    if let Some(v) = &msg.payload {
        if let Some(s) = try_parse_snapshot(v) {
            return Some(s);
        }
        if let Some(s) = nska_extract_snapshot(v) {
            return Some(s);
        }
    }
    for aux in &msg.aux {
...
fn nska_extract_snapshot(v: &Value) -> Option<Snapshot> {
    ...
    match &decoded {
        ...
        Value::Array(arr) => {
            for item in arr {
                if let Some(s) = try_parse_snapshot(item) {
                    return Some(s);
                }
            }
...
fn nska_try_array(v: &Value, objects: &[Value]) -> Option<Snapshot> {
    ...
    for item in arr {
        ...
        if let Some(s) = try_parse_snapshot(&decoded) {
            return Some(s);
        }
    }
    None
...
fn try_parse_snapshot(v: &Value) -> Option<Snapshot> {
    ...
    if !dict.contains_key("SystemCPUUsage") && !dict.contains_key("Processes") {
        return None;
    }
    ...
    let processes: Vec<ProcSnap> = match (schema, proc_dict) {
        (Some(schema), Some(procs)) => {
            ...
            let i_phys = attr_index(schema, "physFootprint")
                .or_else(|| attr_index(schema, "memPhysFootprint"))
                .unwrap_or(4);
            ...
            let mut out: Vec<ProcSnap> = procs
                .values()
                .filter_map(|val| {
                    ...
                    let virt = arr
                        ...
                        .and_then(|v| v.as_unsigned_integer())
                        .unwrap_or(0);
                    let pid = if i_pid < arr.len() {
                        arr[i_pid].as_unsigned_integer().unwrap_or(0) as u32
                    } else {
                        0
                    };
                    let name = if i_name < arr.len() {
                        arr[i_name].as_string().unwrap_or("?").to_string()
                    } else {
                        "?".to_string()
                    };
                    Some(ProcSnap {
...
fn print_htop(snap: &Snapshot, cols: usize, rows: usize) {
    ...
    out.push_str(&format!(
        "\x1b[1miOS Performance Monitor  [{:02}:{:02}:{:02}]\x1b[0m\n",
        secs / 3600,
        (secs % 3600) / 60,
        secs % 60
    ));
    ...
    out.push_str(&format!(
        "CPU: [{}{}] {:5.1}%\n",
        "█".repeat(f1),
        "░".repeat(bar_w.saturating_sub(f1)),
        cpu_pc
    ));

    if snap.phys_mem > 0 {
        let used = if snap.mem_used > 0 {
            snap.mem_used
        } else {
            snap.processes.iter().map(|p| p.phys).sum()
        };
        ...
        out.push_str(&format!(
            "MEM: [{}{}] {} / {}\n",
            "█".repeat(f2),
            "░".repeat(bar_w.saturating_sub(f2)),
            fmt_bytes(used),
            fmt_bytes(snap.phys_mem)
        ));
    }
    ...
    for p in snap.processes.iter().take(rows.saturating_sub(7).max(1)) {
        let name = if p.name.len() > name_w {
            format!("{}…", &p.name[..name_w.saturating_sub(1)])
        } else {
            p.name.clone()
        };
        out.push_str(&format!(
...
fn print_status(msg: &str, _cols: usize) {
    print!(
        "\x1b[H\x1b[2J\x1b[1miOS Performance Monitor\x1b[0m\n{}\n",
        msg
    );
    std::io::stdout().flush().ok();
}
...
fn print_json(snap: &Snapshot) {
    let procs: Vec<String> = snap
        .processes
        .iter()
        .map(|p| {
            format!(
                r#"{{"pid":{},"name":{},"cpu_pct":{:.2},"mem_bytes":{},"virt_bytes":{}}}"#,
                p.pid,
                json_str(&p.name),
                p.cpu * 100.0,
                p.phys,
        ...
        .collect();
    println!(
        r#"{{"ts":{},"cpu_total_pct":{:.2},"phys_mem_bytes":{},"mem_used_bytes":{},"processes":[{}]}}"#,
        snap.ts,
        snap.cpu_total,
        snap.phys_mem,
        snap.mem_used,
