...
pub fn export(udid: Option<&str>, path: Option<&str>, output: OutputMode) -> Result<()> {
    ...
    let raw = conn
        .read_pair_record(&device.serial)
        .context("read pair record from usbmuxd")?;
    let out_path = path
        .map(|s| s.to_owned())
        .unwrap_or_else(|| format!("{}.plist", device.serial));
    std::fs::write(&out_path, &raw).with_context(|| format!("write {out_path}"))?;

    if output.is_json() {
        return print_json(&ExportResult {
            ok: true,
            path: out_path,
        });
    }
...
pub fn import(udid: Option<&str>, path: &str, output: OutputMode) -> Result<()> {
    ...
    let _: plist::Value =
        plist::from_bytes(&raw).with_context(|| format!("{path} is not a valid plist"))?;
    let mut conn = ios_rs::usbmux::Connection::open()?;
    ...
    if output.is_json() {
        return print_json(&ActionResult::with_msg(format!(
            "pair record imported from {path}"
        )));
    }
...
pub fn pair(
    ...
    match (supervision_cert, supervision_key) {
        ...
        (None, None) => {
            ios_rs::lockdown::pairing::pair(device.device_id, &device.serial).context("pairing")?;
...
pub fn unpair(udid: Option<&str>, output: OutputMode) -> Result<()> {
    let device = resolve_device(udid)?;
    ios_rs::lockdown::pairing::unpair(device.device_id, &device.serial).context("unpair")?;
