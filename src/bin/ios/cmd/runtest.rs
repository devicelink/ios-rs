...
use ios_rs::xctest::{RunConfig, run as xctest_run};
...
pub fn run_test(
    ...
    let app_bundles: HashMap<String, ios_rs::rsd::ServiceEntry> = {
        use ios_rs::lockdown::services::{InstallationProxy, AppType};
        ...
        apps.into_iter().map(|app| {
            ...
            props.insert("Path".to_string(), ios_rs::xpc::Value::String(app.path.clone()));
            (app.bundle_id, ios_rs::rsd::ServiceEntry {
                port: 0,
