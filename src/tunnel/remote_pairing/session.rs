...
impl RemotePairingSession {
    pub fn open(
        ...
        let has_creds =
            !record.our.signing_key.is_empty() && !record.peer.peer_identifier.is_empty();
        ...
        if let Some(d) = caps.as_dictionary() {
            eprintln!(
                "[RemotePairing] device responded with keys: {:?}",
    ...
    fn pair_verify(&mut self) -> Result<(), Error> {
        ...
        let dev_pk = {
            ...
            if b.len() != 32 {
                return Err(Error::Protocol("pair-verify: bad key len".into()));
            }
            let mut arr = [0u8; 32];
        ...
        if let Some(e) = tlv.get(&tlv8::ERROR) {
            return Err(Error::Protocol(format!(
                "pair-verify: device error {e:02x?}"
            )));
        }
    ...
    pub fn encrypted_request(
        ...
        msg.insert(
            "sequenceNumber".into(),
            plist::Value::Integer((seq as i64).into()),
        );
        ...
        let cipher_bytes = resp_dict
            ...
            .and_then(|d| d.get("streamEncrypted"))
            .and_then(|v| v.as_dictionary())
            .and_then(|d| d.get("_0"))
            .and_then(|v| v.as_data())
            .ok_or_else(|| Error::Protocol("encrypted response: no ciphertext data".into()))?;

        let resp_seq = resp_dict
            .get("sequenceNumber")
            .and_then(|v| v.as_signed_integer())
            .unwrap_or(seq as i64) as u64;
