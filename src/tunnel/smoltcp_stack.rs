...
use super::cdtunnel::{recv_ipv6, CdTunnelConn, TunnelParams};
...
fn poll_loop(
    ...
    iface.update_ip_addrs(|addrs| {
        let _ = addrs.push(IpCidr::Ipv6(Ipv6Cidr::new(a, 64)));
    });
    ...
    loop {
        ...
        while let Ok(req) = connect_rx.try_recv() {
            ...
            match sock.connect(iface.context(), remote, port) {
                Ok(()) => {
                    pending.push((sockets.add(sock), req.result_tx));
                }
                Err(e) => {
                    let _ = req.result_tx.send(Err(format!("{e:?}")));
                }
            }
        }
