...
fn handle_list_devices(stream: &mut TcpStream, tag: u32, devices: &[SimDevice]) {
    let entries: Vec<plist::Value> = devices
        ...
        .map(|(i, d)| {
            ...
            props.insert(
                "SerialNumber".into(),
                plist::Value::String(d.serial.clone()),
            );
            props.insert(
                "ConnectionType".into(),
                plist::Value::String(d.connection_type.clone()),
            );
            props.insert(
                "ProductID".into(),
                plist::Value::Integer((d.product_id as i64).into()),
            );
            ...
            entry.insert(
                "DeviceID".into(),
                plist::Value::Integer(((i + 1) as i64).into()),
            );
            entry.insert(
                "MessageType".into(),
...
fn handle_listen(stream: &mut TcpStream, tag: u32, devices: &[SimDevice]) {
    ...
    if framing::write_frame(stream, tag, &ok).is_err() {
        return;
    }
    ...
    for (i, d) in devices.iter().enumerate() {
        ...
        if framing::write_frame(stream, 0, &ev).is_err() {
            return;
        }
    }
...
fn handle_connect(stream: &mut TcpStream, tag: u32, payload: &[u8], devices: &[SimDevice]) {
    ...
    if framing::write_frame(stream, tag, &framing::result_plist(0)).is_err() {
        return;
    }
...
fn handle_read_buid(stream: &mut TcpStream, tag: u32) {
    ...
    d.insert(
        "BUID".into(),
        plist::Value::String("DEADBEEF-0000-0000-0000-CAFECAFECAFE".into()),
    );
    let mut buf = Vec::new();
