...
use super::plist_msg::{self, BaseRequest, ConnectRequest, PairRecordRequest, SavePairRecordRequest};
use super::types::{ConnectionType, Device, Event, ResultCode};
...
impl Codec {
    ...
    pub fn poll_write(&mut self) -> Option<Vec<u8>> { self.tx.pop_front() }
    ...
    fn process_rx(&mut self) {
        loop {
            ...
            if total < HEADER_LEN || self.rx.len() < total { return; }
    ...
    fn decode_frame(&mut self, tag: u32, payload: &[u8]) -> Option<Event> {
        let env = plist_msg::decode(payload).ok()?;
        ...
        match msg_type {
            ...
            "" => {
                if let Some(list) = env.device_list {
                    return Some(Event::DeviceList(list.into_iter().map(device_from_entry).collect()));
                }
                if let Some(buid) = env.buid { return Some(Event::Buid(buid)); }
                if let Some(data) = env.pair_record_data {
                    return Some(Event::PairRecord { udid: String::new(), record: data.into_vec() });
                }
                None
            }
            "Attached" => {
                ...
                Some(Event::DeviceAttached(Device {
                    ...
                    connection_type: ConnectionType::from(props.connection_type.as_deref().unwrap_or("")),
                    product_id:      props.product_id.unwrap_or(0),
                    location_id:     props.location_id.unwrap_or(0),
                }))
            }
            "Detached" => Some(Event::DeviceDetached { device_id: env.device_id? }),
            _ => None,
        }
    }
}
...
fn device_from_entry(e: plist_msg::DeviceEntry) -> Device {
    Device {
        ...
        connection_type: ConnectionType::from(e.properties.connection_type.as_deref().unwrap_or("")),
...
mod tests {
    ...
    fn frame_for(body: &[u8], tag: u32) -> Vec<u8> {
        let total = (HEADER_LEN + body.len()) as u32;
        let mut f = Vec::new();
        f.extend_from_slice(&total.to_le_bytes());
        f.extend_from_slice(&VERSION_PLIST.to_le_bytes());
        f.extend_from_slice(&MSGTYPE_PLIST.to_le_bytes());
        f.extend_from_slice(&tag.to_le_bytes());
        f.extend_from_slice(body);
        f
    }

    fn list_devices_response() -> Vec<u8> {
...
<plist version="1.0"><dict>
  <key>DeviceList</key><array><dict>
    ...
    <key>Properties</key><dict>
      ...
      <key>LocationID</key><integer>336592896</integer>
    </dict>
  </dict></array>
</dict></plist>"#;
        frame_for(body, 1)
    ...
    fn list_devices_decodes_response() {
        let mut codec = Codec::new();
        let _ = codec.list_devices(); let _ = codec.poll_write();
        codec.push_data(&list_devices_response());
    ...
    fn partial_frame_buffered() {
        let mut codec = Codec::new();
        let _ = codec.list_devices(); let _ = codec.poll_write();
        let frame = list_devices_response();
        let mid = frame.len() / 2;
    ...
    fn connect_emits_connected_on_ok() {
        let result_body = br#"<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
  ...
  <key>Number</key><integer>0</integer>
        ...
        let mut codec = Codec::new();
        let tag = codec.connect(5, 62078); let _ = codec.poll_write();
        codec.push_data(&frame_for(result_body, tag));
        match codec.poll_event().unwrap() {
