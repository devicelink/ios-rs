...
use super::plist_msg::{
    self, BaseRequest, ConnectRequest, PairRecordRequest, SavePairRecordRequest,
};
...
impl Codec {
    ...
    fn process_rx(&mut self) {
        loop {
            ...
            if total < HEADER_LEN || self.rx.len() < total {
                return;
            }
        ...
        match msg_type {
            ...
            "" => {
                if let Some(list) = env.device_list {
                    return Some(Event::DeviceList(
                        list.into_iter().map(device_from_entry).collect(),
                    ));
                }
